...
export async function deployHistoryRoutes(app: FastifyInstance) {

  // GET /api/v1/deploys?appId=xxx — List deploys for an app
  app.get('/deploys', async (request) => {
    const { appId, limit } = request.query as { appId?: string; limit?: string };
    const user = getRequestUser(request);

    const where: any = {};
    if (appId) where.appId = appId;
    if (user.role !== 'admin') where.app = { ownerId: user.id };
