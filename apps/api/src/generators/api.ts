...
// ─── Field Validator ─────────────────────────────────────────────
function validateField(value: any, field: FieldConfig): string | null {
  ...
  switch (field.type) {
    ...
    case 'text':
      ...
      if (field.minLength && value.length < field.minLength) return `${field.name} must be at least ${field.minLength} characters`;
      if (field.maxLength && value.length > field.maxLength) return `${field.name} must be at most ${field.maxLength} characters`;
      break;
    case 'number':
    case 'currency':
      if (typeof value !== 'number') return `${field.name} must be a number`;
      if (field.min !== undefined && value < field.min) return `${field.name} must be >= ${field.min}`;
      if (field.max !== undefined && value > field.max) return `${field.name} must be <= ${field.max}`;
      break;
    case 'boolean':
      if (typeof value !== 'boolean') return `${field.name} must be a boolean`;
      break;
    case 'email':
      if (typeof value !== 'string' || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(value)) return `${field.name} must be a valid email`;
      break;
    case 'enum':
      if (field.enumValues && !field.enumValues.includes(value)) return `${field.name} must be one of: ${field.enumValues.join(', ')}`;
      break;
    case 'date':
      if (isNaN(Date.parse(value))) return `${field.name} must be a valid date`;
      break;
    case 'url':
      try { new URL(value); } catch { return `${field.name} must be a valid URL`; }
      break;
  }
  return null;
}

function validateRecord(data: Record<string, any>, fields: FieldConfig[]): { valid: boolean; errors: string[] } {
  const errors: string[] = [];
...
function getStore(tenantId: string, entity: string): any[] {
...
// ─── Route Generator ─────────────────────────────────────────────
export function generateEntityRoutes(app: FastifyInstance, entity: EntityConfig, tenantId: string = 'default') {
  ...
  app.get(prefix, async (request) => {
    ...
    if (query.search) {
      ...
      records = records.filter(r =>
        entity.fields.some(f => String(r[f.name] || '').toLowerCase().includes(search))
      );
    }
  ...
  app.get(`${prefix}/:id`, async (request, reply) => {
    ...
    if (!record) return reply.status(404).send({ success: false, error: { code: 'NOT_FOUND', message: `${entity.label || entity.name} not found` } });
  ...
  app.post(prefix, async (request, reply) => {
    ...
    if (!valid) return reply.status(400).send({ success: false, error: { code: 'VALIDATION_ERROR', message: 'Validation failed', details: errors } });
  ...
  app.put(`${prefix}/:id`, async (request, reply) => {
    ...
    if (index === -1) return reply.status(404).send({ success: false, error: { code: 'NOT_FOUND', message: 'Record not found' } });
  ...
  app.post(`${prefix}/bulk`, async (request, reply) => {
    ...
    if (!Array.isArray(body.records)) return reply.status(400).send({ success: false, error: { code: 'INVALID_BODY', message: 'body.records must be an array' } });
...
export function generateAllEntityRoutes(app: FastifyInstance, entities: EntityConfig[], tenantId = 'default') {
